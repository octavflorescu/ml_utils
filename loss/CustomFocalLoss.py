# ADAPTED VERISON OF balanced_loss pip module
...
def focal_loss(logits, labels, alpha=None, gamma=2):
    ...
    custom_scaling = torch.ones(labels.shape).to(device)
    custom_scaling = custom_scaling.where((labels[:,1]<1).reshape(-1,1), torch.tensor([20,20]).to(device))
    loss = modulator * bc_loss * custom_scaling
...
class CustomFLoss(torch.nn.Module):
    def __init__(
        self,
        ...
        Returns:
            Loss instance
        """
        super(CustomFLoss, self).__init__()
